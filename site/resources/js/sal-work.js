...
function myScrollIntoView (targetId) {
  ...
  let goHere = offset - parseInt($('div.navbar-white').css('height')) - 15
...
$('a[href*="#"]').click(function (event) {
  let z = $(this).attr('href').slice($(this).attr('href').indexOf('#') + 1)
  if ($(z).length !== 0) {
...
ias.on('page', (event) => { // update the url to reflect the new page we've just scrolled to
  ...
  let newUrl = target.pathname.substr(target.pathname.lastIndexOf('/') + 1) + newParams
