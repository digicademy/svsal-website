...
function myScrollIntoView(targetId) {
  ...
  console.log(
    `document.getElementById('${targetId}').offset().top = ${offset}.`,
  );
...
ias.on("page", (event) => {
  ...
  ); // event.url is a string, but we want to use URL methods (second parameter is basename)
  ...
  const newUrl =
    target.pathname.substr(target.pathname.lastIndexOf("/") + 1) + "?" + params;
...
document.body.addEventListener("click", async function (e) {
  ...
  if (t.matches('a[href*="#W"]')) {
    ...
    t.matches(".dropdown-menu.export-options.dropdown-toggle.dropdown")
  ) {
