(function () {
  ...
  var defaults = {
    ...
    listItemTemplate:
      ...
      '<div class="facetitem" id="<%= id %>"> <div class="facetitem_container">Reference Works <span class="facetitemcount">(<%= count %>)</span>  <div type="button" style="margin-left: 4px;" class="glyphicon glyphicon-info-sign" data-toggle="tooltip"  data-placement="right" title="Transcribed texts with technical editing only"></div></div></div>' +
      '<%} else if (name == "Editions") { %>' +
      '<div class="facetitem" id="<%= id %>"> <div class="facetitem_container"><%= name %> <span class="facetitemcount">(<%= count %>)</span>  <div type="button" style="margin-left: 4px;" class="glyphicon glyphicon-info-sign" data-toggle="tooltip"  data-placement="right" title="Fully edited texts"></div></div></div>' +
      '<% } else if (name == "Facsimiles") { %>' +
      '<div class="facetitem" id="<%= id %>"><div class="facetitem_container"><div class="facetitem_text"><%= name %> <span class="facetitemcount">(<%= count %>)</span> </div> <div type="button" style="margin-left: 4px;" class="glyphicon glyphicon-info-sign" data-toggle="tooltip"  data-placement="right" title="Image scans only; text is forthcoming"></div></div></div>' +
      "<% } else { %>" +
      '<div class="facetitem" id="<%= id %>"> <div class="facetitem_container"><%= name %> <span class="facetitemcount">(<%= count %>)</span> </div></div>' +
      "<% }%>",
    bottomContainer: "<sal></sal>",
    orderByTemplate:
      '<div class="orderby" style="margin-left: -12%; margin-top:-25%; padding-bottom: 10%"> <div class="dropdown"><button class="btn btn-link btn-xlarge dropdown-toggle" id="order" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-expanded="true"><span class="glyphicon glyphicon-sort-by-alphabet" aria-hidden="true"></span> &nbsp;<span class="caret"></span></button><ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1">' +
      "<% _.each(options, function(value, key) { %>" +
      "<li role=presentation><a role=menuitem tabindex=-1 class=orderbyitem id=orderby_<%= key %>>" +
      "<%= value %></a></li> <% }); %></ul></div>",
    countTemplate:
      '<div class=facettotalcount style="position: absolute; top: 10px; left:340px;width:400px"><%= count %> Items&#xA0;</div>',
    deselectTemplate:
      '<div class="deselectstartover" style="position: absolute;top: 15%;"><span class="glyphicon glyphicon-remove-circle"></span> Deselect all filters</div>',
    resultTemplate: "<div class=facetresultbox><%= name %></div>",
    noResults:
      "<div class=results style=float:left;>Sorry, but no items match these criteria (or the list of items has not been found [hint to admin])</div>",
    ...
    showMoreTemplate:
      '<a id="showmorebutton"><div class="text-center"><a class="btn btn-default">&nbsp;&nbsp;<i class="fa fa-chevron-right"></i>&nbsp;&nbsp;</a></div></a>',
    showAllTemplate:
      ' <a id="showallbutton"><div class"text-center"><a class="btn btn-primary">View All</a></div></a>',
      ...
      _.each(settings.facets, function (facettitle, facet) {
        if ($.isArray(item[facet])) {
          _.each(item[facet], function (facetitem) {
            settings.facetStore[facet][facetitem] = settings.facetStore[facet][
              facetitem
            ] || { count: 0, id: _.uniqueId("facet_") };
          });
        } else {
          if (item[facet] !== undefined) {
            settings.facetStore[facet][item[facet]] = settings.facetStore[
              facet
            ][item[facet]] || { count: 0, id: _.uniqueId("facet_") };
          }
        }
      });
    });
  ...
  function order() {
    if (settings.state.orderBy) {
      ...
      settings.currentResults = _.sortBy(
        settings.currentResults,
        function (item) {
  ...
  function toggleFilter(key, value) {
    ...
    if (_.indexOf(settings.state.filters[key], value) == -1) {
      ...
      settings.state.filters[key] = _.without(
        settings.state.filters[key],
        value,
      );
  ...
  function createFacetUI() {
    ...
    $(".facetitem, .refWork_facet").click(function (event) {
  ...
  function updateFacetUI() {
    ...
    _.each(settings.facetStore, function (facet, facetname) {
      _.each(facet, function (filter, filtername) {
        ...
        if (
          settings.state.filters[facetname] &&
          _.indexOf(settings.state.filters[facetname], filtername) >= 0
        ) {
  ...
  function updateResults() {
    $(settings.resultSelector).html(
      settings.currentResults.length == 0 ? settings.noResults : "",
    );
    showMoreResults();
  }

  //08.2024: add here an option to show all the datas at once
  var moreButton;
  var allButton;

  function showAllResults() {
    allButton = $(settings.showAllTemplate).click(showAllResults);
    var itemHtml = "";
  ...
  function showMoreResults() {
    var showNowCount = settings.enablePagination
      ? Math.min(
          settings.currentResults.length - settings.state.shownResults,
          settings.paginationCount,
        )
      : settings.currentResults.length;
    var itemHtml = "";
    var template = _.template(settings.resultTemplate);
    for (
      var i = settings.state.shownResults;
      i < settings.state.shownResults + showNowCount;
      i++
    ) {
