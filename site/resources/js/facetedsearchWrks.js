...
listItemTemplate   :  '<% if (name == "Reference Work") { %>' + '<div class="facetitem" id="<%= id %>"> <div class="facetitem_container">Reference Works <span class="facetitemcount"> (<%= count %>) </span>  <div type="button" style="margin-left: 4px;" class="glyphicon glyphicon-info-sign" data-toggle="tooltip"  data-placement="right" title="Transcribed texts with technical editing only"></div></div></div>' + '<%} else if (name == "Edited Work") { %>' + '<div class="facetitem" id="<%= id %>"> <div class="facetitem_container">Scholarly Edited Works <span class="facetitemcount"> (<%= count %>) </span>  <div type="button" style="margin-left: 4px;" class="glyphicon glyphicon-info-sign" data-toggle="tooltip"  data-placement="right" title="Fully edited texts"></div></div></div>' +    '<% } else if (name == "Automatically Edited Work") { %>' + '<div class="facetitem" id="<%= id %>"><div class="facetitem_container"><div class="facetitem_text">Automatically Edited Works <span class="facetitemcount"> (<%= count %>) </span> </div> <div type="button" style="margin-left: 4px;" class="glyphicon glyphicon-info-sign" data-toggle="tooltip"  data-placement="right" title="Transcribed works with technical editing only"></div></div></div>'  +'<% } else if (name == "Facsimiles") { %>' + '<div class="facetitem" id="<%= id %>"><div class="facetitem_container"><div class="facetitem_text"><%= name %> <span class="facetitemcount">(<%= count %>)</span> </div> <div type="button" style="margin-left: 4px;" class="glyphicon glyphicon-info-sign" data-toggle="tooltip"  data-placement="right" title="Image scans only; text is forthcoming"></div></div></div>' +'<% } else { %>' + '<div class="facetitem" id="<%= id %>"> <div class="facetitem_container"><%= name %> <span class="facetitemcount">(<%= count %>)</span> </div></div>' +'<% }%>',
  bottomContainer    : '<sal></sal>',
  orderByTemplate    :  '<div class="orderby" style="margin-left: -12%; margin-top:-25%; padding-bottom: 10%"> <div class="dropdown"><button class="btn btn-link btn-xlarge dropdown-toggle" id="order" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-expanded="true"><span class="glyphicon glyphicon-sort-by-alphabet" aria-hidden="true"></span> &nbsp;<span class="caret"></span></button><ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1">'+
                        '<% _.each(options, function(value, key) { %>'+
                        '<li role=presentation><a role=menuitem tabindex=-1 class=orderbyitem id=orderby_<%= key %>>'+
                        '<%= value %></a></li> <% }); %></ul></div>',   
  countTemplate      : '<div class=facettotalcount style="position: absolute; top: 10px; left:340px;width:400px"><%= count %> Items&#xA0;</div>',
  deselectTemplate   : '<div class="deselectstartover" style="position: absolute;top: 15%;"><span class="glyphicon glyphicon-remove-circle"></span> Deselect all filters</div>',
  ...
  showAllTemplate      : ' <a id="showallbutton"><div class"text-center"><a class="btn btn-primary">View All</a></div></a>',
