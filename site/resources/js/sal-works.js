var item_template =
  ...
  '<p class="typeWork"><%= obj.type %>  <span type="button" style="margin-left: 4px;" class="glyphicon glyphicon-info-sign" data-toggle="tooltip" data-placement="right" title="Transcribed texts with technical editing only"></span></p>' +
  "</br>" +
  '<a href="<%= obj.monoMultiUrl %>"><span class="work-title"><% if (obj.title) {  %><%= obj.title %><% } %> </span></a>' +
  '<% } else if (obj.type == "Facsimiles") { %>' +
  '<p class="typeWork"><%= obj.type %>  <span type="button" style="margin-left: 4px;" class="glyphicon glyphicon-info-sign" data-toggle="tooltip" data-placement="right" title="Image scans only; text is forthcoming"></span></p>' +
  "</br>" +
  '<a href="<%= obj.monoMultiUrl %>"><span class="work-title"><% if (obj.title) {  %><%= obj.title %><% } %> </span></a>' +
  "<% } else {%>" +
  ' <p class="typeWork"><%= obj.type %>  <span type="button" style="margin-left: 4px;" class="glyphicon glyphicon-info-sign" data-toggle="tooltip" data-placement="right" title="Fully edited texts"></span></p>' +
  "</br>" +
  '<a href="<%= obj.monoMultiUrl %>"><span class="work-title"><% if (obj.title) {  %><%= obj.title %><% } %> </span></a>' +
  "<% }%>" +
  "</br>" +
...
var settings = {
  ...
  deselectTemplate:
    '<div class=deselectstartover><span class="glyphicon glyphicon-remove-circle"></span> Deselect filters</div>',
  countTemplate:
    '<div class=facettotalcount style="position: absolute; top: 10px; left:340px;width:400px"><%= count %> Entries </div>',
  noResults:
