...
'<a href="<%= obj.monoMultiUrl %>"><span class="work-title"><% if (obj.title) {  %><%= obj.title %><% } %> </span></a>' +
 ...
 '<% } else { %>'+
' <p class="typeWork"> Other <span type="button" style="margin-left: 4px;" class="glyphicon glyphicon-info-sign" data-toggle="tooltip" data-placement="right" title="No type found"></spa    n></p>'+ '</br>' +
'<a href="<%= obj.monoMultiUrl %>"><span class="work-title"><% if (obj.title) {  %><%= obj.title %><% } %> </span></a>' + 
 '<% }%>' +

						'</br>' +
						'<span><% if (obj.name) {  %><%= obj.name %><% } %></span>' +'</div>'+
					...
					'<p class="volumeLinks">' +
						...
						'<% }%>' +
					'</p>' +'<% if (obj.type == "Reference Work") { %>' +
						'<a title="<%=obj.facsAttrib %>" class="workImages pull-right" target="_blank" rel="noopener noreferrer" href="<%= obj.workImages %>"><i class="fas fa-book-open" ></i> <%=obj.facsAttrib %></a>' + '<% } else {%>' + '<a title="<%=obj.facsAttrib %>" class="workImages pull-right" target="_blank" rel="noopener noreferrer" href="<%= obj.workImages %>"><i class="fas fa-book-open" ></i> <%=obj.facsAttrib %></a>'  + '<% }%>' + 
					'<br/>' + 
 '<% if (obj.type == "Reference Work") { %>'+ '<a title="<%=obj.titAttrib %>" class="workDetails pull-right" href="<%= obj.workdetails %>"><i class="fas fa-file-alt" ></i> <%=obj.titAttrib %></a>' +
                                                '<a style="display: none;"  ><%= obj.sortName %></a>'  +  '<% } else {%>'  +
