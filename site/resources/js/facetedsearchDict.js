...
;(function () {
  ...
  var defaults = {
    ...
    orderByTemplate: '<div class="orderby" style="margin-left: -12%; margin-top:-120%; padding-bottom: 10%">' +
                      '<div class="dropdown">' +
                        '<button class="btn btn-link btn-xlarge dropdown-toggle" id="order" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-expanded="true">' +
                          '<span class="glyphicon glyphicon-sort-by-alphabet" aria-hidden="true"></span> &nbsp;<span class="caret"></span>' +
                        '</button>' +
                        '<ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1">' +
                          '<% _.each(options, function(value, key) { %>' +
                            '<li role="presentation">' +
                              '<a role="menuitem" tabindex="-1" class="orderbyitem" id="orderby_<%= key %>"><%= value %></a>' +
                            '</li>' +
                          '<% }); %>' +
                        '</ul>' +
                      '</div>' +
  ...
  function createFacetUI () {
    ...
    let countHtml = _.template(settings.countTemplate, {count: settings.currentResults.length})
